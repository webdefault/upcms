...
class ECMColorlistField extends SimpletextField
{
	...
	{
		$from = $this->getFrom( $this->field->from );

		$sql = 'SELECT GROUP_CONCAT( '.$this->column().' SEPARATOR \'|\') FROM '.$from->table.' WHERE ';
		$append = '';
	...
	public function listView( $id, $values )
	{
		...
		$colors = explode( '|', $values[$this->fieldName] );
		foreach( $colors AS $c )
		{
			$color = new stdClass();
			$color->type = 'thumb';
...
global $__CMSFields;
$__CMSFields['ecm-colorlist'] = 'ECMColorlistField';
