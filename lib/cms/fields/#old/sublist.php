...
class Sublist extends SimpletextField
{
	...
	protected function loadMapValues( $id )
	{
		...
		$subrequest = Request::createRequestFromTarget( $this->field->map, $this->submap, array(), array(), $this->db );
		
		$custom = new CustomField( $this->field->map, $join[0], '"'.$id.'"' );
		$custom->set( $subrequest, $this->field->map, $subrequest, array(), array(), NULL, $this->db );
	...
	public function listView( $id, $values )
	{
		...
		foreach( $list as $line )
		{
			$row = array(
				'id' => $line['id'], 
				'click' => $page->list->{'open-edit-as'}.'://edit-content/'.$this->field->map.'/'.$line['id'], 
				'columns' =>array() 
				);
